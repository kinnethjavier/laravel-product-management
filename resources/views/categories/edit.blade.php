...
<div class="bg-white pt-12 pb-2">
    <div class="mx-auto px-4 py-16 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        @if($categoryInfo)
        <form class="grid grid-cols-2 gap-x-20 gap-y-6" action="{{ route('categories.update', $categoryInfo->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-6 col-span-2 lg:col-span-1">
                ...
                <div>
                    ...
                    <div class="mt-2">
                    <input id="category" name="category" type="text" required value="{{ $categoryInfo->category }}" class="block w-full rounded-md border-0 px-2.5 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 outline-0 focus:ring-2 focus:ring-inset focus:ring-primary sm:leading-6">
                    </div>
                </div>
                <!-- Description -->
                <div>
                    ...
                    <div class="mt-2">
                    <textarea rows="6" id="description" name="description" class="block w-full rounded-md border-0 px-2.5 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 outline-0 focus:ring-2 focus:ring-inset focus:ring-primary sm:leading-6">{{ $categoryInfo->description }}</textarea>
                    </div>
                </div>
